...
# 5. 提交；精确小写标记 [pub-page] 或 [pub page] 用于触发部署
git commit -m "feat: 发布 v0.1.x Web H5 [pub-page]"
...
提交时 commit message 包含 `[pub-page]` 或 `[pub page]`，推送后自动触发部署任务：
...
> GitLab Pages 由 `.gitlab-ci.yml` 独立触发，推送到 GitLab 的 `main` 分支且 commit 含 `[pub-page]` 或 `[pub page]` 时自动部署。
...
- **触发器**：推送到 GitLab `main` 分支 + commit 含 `[pub-page]` 或 `[pub page]`
...
确认推送目标是 GitHub `main`，并用 `git log -1 --format=fuller` 检查 HEAD 提交信息是否包含精确小写 `[pub-page]` 或 `[pub page]`。
